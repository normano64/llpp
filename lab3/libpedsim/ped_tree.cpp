...
void  Ped::Ttree::doSafeMovement(std::set<const Ped::Tagent*>& betweenRegions ) {
  ...
  if(isleaf)
      ...
      for(it1 = agents.begin(); it1 != agents.end(); it1++)
        {
		  ...
		  if(!inside(agent->getDesiredX(), agent->getDesiredY()))
		  {
		  ...
		  if(!(inside(p1.first, p1.second) && inside(p2.first, p2.second)))
			{
			  betweenRegions.insert(agent);
			  continue;
			}
    ...
    {
#pragma omp parallel sections shared(betweenRegions)
	  {
		  #pragma omp section
		  tree1->doSafeMovement(betweenRegions);
		  #pragma omp section
		  tree2->doSafeMovement(betweenRegions);
		  #pragma omp section
		  tree3->doSafeMovement(betweenRegions);
#pragma omp section
		  tree4->doSafeMovement(betweenRegions);
	  }
	}
}
