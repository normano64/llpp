...
const char *kernelsource = "                                               \
  __kernel void go(__global float *agents,                                \
                   __global float *waypoints,                             \
                   __global char *reached) {                               \
    int i = get_global_id(0);                                              \
    int iA = i * 2;                                                        \
    int iW = i * 3;                                                        \
    float waypointX = waypoints[iW] - agents[iA];                         \
    float waypointY = waypoints[iW+1] - agents[iA+1];                     \
    float length = sqrt(waypointX*waypointX + waypointY*waypointY);       \
...
void Ped::Model::setup(vector<Ped::Tagent*> agentsInScenario, IMPLEMENTATION impl)
{
    ...
    if(implementation == OPENCL) {
        ...
        // tempagentsX = new float[length];
        tempagents = new float[length*2];
        waypoints = new float[length*3];
        reached = new char[length];
        //waypointY = new float[length];
        // waypointR = new float[length];
        ...
        sizeA = length * sizeof(float) * 2;
        sizeW = length * sizeof(float) * 3;
